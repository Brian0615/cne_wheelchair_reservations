...
from common.constants import Location, RentalStatus
...
date, rental_id, rental_data = get_rental_selection(
    ...
    in_progress_rentals_only=False,
    key_prefix="manage_rental",
)
if rental_id is None:
    st.stop()

form_col, update_col = st.columns([1, 2])

with form_col:
    with st.expander("Download Rental Form", expanded=True):
        status_code, form_data = data_service.download_rental_form(rental_id=rental_id)
        if status_code == 200:
            st.download_button(
                "Download",
            ...
            st.error(
                f"**Error**: Unable to find rental form. "
                f"Status code: {status_code}. Please try again later."
            )

with update_col:
    with st.expander(f"Update Rental {rental_data['device_type'].value}", expanded=True):
        if rental_data["status"] == RentalStatus.COMPLETED:
            st.warning(
                "**Note**: This rental has already been completed. "
                "You may not change the device for a completed rental."
            )
            st.stop()
        change_device_info = {
